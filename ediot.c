...
void disableRawMode(){
	if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &E.termios_orig) == -1){ 
		die("tcsetattr");
	}
}

//Function to allow terminal to enter the raw mode
void enableRawMode(){
	if(tcgetattr(STDIN_FILENO, &E.termios_orig) == -1){ 
		die("tcgetattr");
	}
	atexit(disableRawMode);

	struct termios raw 	= E.termios_orig;
	
	//Set flags for termios struct to allow the terminal interface to behave like we want
...
void editorUpdateSyntax(editorRow *row){
	...
	int prev_sep = 1;
	int in_string = 0;
	int in_comment = (row->idx > 0 && E.row[row->idx - 1]);
...
int editorRowCxToRx(editorRow *row, int cx){
	...
	for (j = 0; j<cx; j++) {
		if(row->chars[j] == '\t'){
			rx += (EDIOT_TAB_STOP - 1) - (rx % EDIOT_TAB_STOP);
		}
		rx++;
	}
	return rx;
...
int editorRowRxToCx(editorRow *row, int rx){
	...
	for(cx = 0; cx < row->size; cx++){
		if(row->chars[cx] == '\t'){
			cur_rx += (EDIOT_TAB_STOP - 1) - (cur_rx % EDIOT_TAB_STOP);
		}
		cur_rx++;
...
void editorUpdateRow(editorRow *row){
	...
	free(row->render);
	row->render 					= malloc(row->size + tabs*(EDIOT_TAB_STOP-1) + 1);

	int idx = 0;
	for (j = 0; j < row->size; j++) {
		if(row->chars[j] == '\t'){
			row->render[idx++] 		= ' ';
			while (idx%EDIOT_TAB_STOP != 0){
				row->render[idx++] 	= ' ';
			}
		}else{
	...
	for (y = 0; y < E.screenrows; y++){
		...
		if(filerow >= E.numrows){	
			if(E.numrows == 0 && y == E.screenrows / 3){
				char welcome[80];
				int welcomelen = snprintf(welcome, sizeof(welcome), "Ediot editor -- version %s", EDIOT_VERSION);
		...
		//Write last line tilde without buffer deletion
		abAppend(ab, "\x1b[K", 3);
		abAppend(ab, "\r\n", 2);
	}
}
...
void editorProcessKeypress(){
	static int quit_times 				= EDIOT_QUIT_TIMES;
